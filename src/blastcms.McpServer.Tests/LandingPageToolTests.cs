...
[TestFixture]
public class LandingPageToolTests
{
    ...
    [Test]
    public async Task ListTools_ReturnsExpectedLandingPageTools()
    {
        ...
        await using (client)
        {
            ...
            Assert.That(tools.Any(t => t.Name == "list_landing_pages"), Is.True,
                "Expected list_landing_pages tool to be registered");
            Assert.That(tools.Any(t => t.Name == "get_landing_page_by_slug"), Is.True,
                "Expected get_landing_page_by_slug tool to be registered");
            Assert.That(tools.Any(t => t.Name == "get_landing_page_by_id"), Is.True,
                "Expected get_landing_page_by_id tool to be registered");
        }
    ...
    [Test]
    public async Task ListLandingPages_WithValidApiKey_ReturnsLandingPageList()
    {
        const string expectedJson = "{\"data\":[{\"id\":\"00000000-0000-0000-0000-000000000001\",\"title\":\"Home Page\",\"slug\":\"home\"}],\"count\":1,\"page\":1}";
        var handler = CreateSuccessHandler(expectedJson);
        var (client, serverTask, cts) = await CreateClientServerPair(handler);

        await using (client)
        {
            var result = await client.CallToolAsync("list_landing_pages",
            ...
            Assert.That(textContent!.Text, Does.Contain("Home Page"));
        }
    ...
    [Test]
    public async Task GetLandingPageBySlug_WithValidApiKey_ReturnsLandingPage()
    {
        const string expectedJson = "{\"id\":\"00000000-0000-0000-0000-000000000001\",\"title\":\"Home Page\",\"slug\":\"home\"}";
        var handler = CreateSuccessHandler(expectedJson);
        var (client, serverTask, cts) = await CreateClientServerPair(handler);

        await using (client)
        {
            var result = await client.CallToolAsync("get_landing_page_by_slug",
                new Dictionary<string, object?> { ["slug"] = "home" });

            Assert.That(result, Is.Not.Null);
            Assert.That(result.IsError, Is.Not.True, "Expected successful result");
            var textContent = result.Content.OfType<TextContentBlock>().FirstOrDefault();
            Assert.That(textContent, Is.Not.Null, "Expected text content in result");
            Assert.That(textContent!.Text, Does.Contain("home"));
        }
    ...
    [Test]
    public async Task GetLandingPageById_WithValidApiKey_ReturnsLandingPage()
    {
        const string expectedJson = "{\"id\":\"00000000-0000-0000-0000-000000000001\",\"title\":\"Home Page\",\"slug\":\"home\"}";
        var handler = CreateSuccessHandler(expectedJson);
        var (client, serverTask, cts) = await CreateClientServerPair(handler);
        ...
        await using (client)
        {
            var result = await client.CallToolAsync("get_landing_page_by_id",
    ...
    [Test]
    public async Task ListLandingPages_WithInvalidApiKey_ReturnsError()
    {
