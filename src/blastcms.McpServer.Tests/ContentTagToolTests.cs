...
[TestFixture]
public class ContentTagToolTests
{
    ...
    [Test]
    public async Task ListTools_ReturnsExpectedContentTagTools()
    {
        ...
        await using (client)
        {
            ...
            Assert.That(tools.Any(t => t.Name == "list_content_tags"), Is.True,
                "Expected list_content_tags tool to be registered");
        }
    ...
    [Test]
    public async Task ListContentTags_WithValidApiKey_ReturnsContentTagList()
    {
        const string expectedJson = "{\"data\":[{\"id\":\"00000000-0000-0000-0000-000000000001\",\"name\":\"Technology\",\"slug\":\"technology\"}],\"count\":1,\"page\":1}";
        var handler = CreateSuccessHandler(expectedJson);
        var (client, serverTask, cts) = await CreateClientServerPair(handler);

        await using (client)
        {
            var result = await client.CallToolAsync("list_content_tags",
            ...
            Assert.That(textContent!.Text, Does.Contain("Technology"));
        }
    ...
    [Test]
    public async Task ListContentTags_WithInvalidApiKey_ReturnsError()
    {
