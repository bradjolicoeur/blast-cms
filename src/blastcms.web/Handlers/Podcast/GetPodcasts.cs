...
namespace blastcms.web.Handlers
{
    public class GetPodcasts
    {
        ...
        public class PagedData : IPagedData<Podcast>
        {
            public PagedData(IEnumerable<Podcast> data, long count, int page)
            {
                Data = data;
                Count = count;
                Page = page;
            ...
            public IEnumerable<Podcast> Data { get; }
        ...
        public class Handler : IRequestHandler<Query, PagedData>
        {
            ...
            public async Task<PagedData> Handle(Query request, CancellationToken cancellationToken)
            {
                using var session = _sessionFactory.QuerySession();
                {
                    ...
                    var query = session.Query<Podcast>()
                        ...
                        .If(!string.IsNullOrWhiteSpace(request.Search), x => x.Where(q => q.Title.Contains(request.Search, StringComparison.OrdinalIgnoreCase)
                                || q.OwnerName.Contains(request.Search, StringComparison.OrdinalIgnoreCase)
